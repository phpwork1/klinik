...
class AuthAssignment extends \yii\db\ActiveRecord
{
    ...
    */
    public static function getAll($value = 'name', $conditions = null) {
        $query = AuthAssignment::find()->orderBy([$value => SORT_ASC]);
        if ($conditions != null) {
            $query->andWhere();
        }
        return $query->all();
    ...
    */
    public static function map($key = 'id', $value = 'name', $conditions = null) {
        ...
        $map = ArrayHelper::map(self::getAll($key, $value, $conditions), $key, $value);
        if (empty($map)) {
            Yii::$app->session->setFlash('danger', Yii::t('app', 'AuthAssignment database still empty. Please add the data as soon as possible.'));
        }
        return $map;
    }
